...
const main = async () => {
    ...
    for (var i = 0; i < tokens.length; i++) {
        const { token0Contract, token1Contract, token0, token1 } = await getTokenAndContract(tokens[i].TOKEN_TO_BORROW, tokens[i].TOKEN_TO_SWAP)
        ...
        sPair.events.Swap({}, async () => {
            try {
                if (!isExecuting) {
                    ...
                    //const receipt = await executeTrade(routerPath, token0Contract, token1Contract)
                    console.log(`Arbitrage Currently Available\n`)
                    console.log(`-----------------------------------------\n`)
                    isExecuting = false
                }
            }
...
const determineProfitability = async (_routerPath, _token0Contract, _token0, _token1) => {
    ...
    // This is a basic example of trading WETH/...

    let reserves, exchangeToBuy, exchangeToSell
    reservesSushi = await getReserves(sPair)
    reservesUni = await getReserves(uPair)

    if (_routerPath[0]._address == uRouter._address) {
        console.log("Sushi Pair: " + sPair._address);
        reserves = await getReserves(sPair)
        exchangeToBuy = 'Uniswap'
        exchangeToSell = 'Sushiswap'

        if (parseFloat(reserves[0]) > parseFloat(reservesUni[0])) {
            reserves[0] = reservesUni[0];
        }
    } else {

        console.log("Uniswap Pair: " + uPair._address);

        reserves = await getReserves(uPair)
        console.log("Emaior?" + reserves[0] > reservesSushi[0]);
        if (parseFloat(reserves[0]) > parseFloat(reservesSushi[0])) {
            reserves[0] = reservesSushi[0];
        }
        exchangeToBuy = 'Sushiswap'
        exchangeToSell = 'Uniswap'
    }

    console.log(`Reserves on Sushi ${reservesSushi[0]}`)
    console.log(`Reserves on Uni ${reservesUni[0]}`)
    ...
    try {
        ...
        console.log("Test1: " + _routerPath[0]._address)
